...
then
	...
	az deployment sub create \
		--location eastus \
		--template-uri "$githubUri/roleDefinition/create/deploy.json?token=***REMOVED***" \
		--parameters \
			"$githubUri/roleDefinition/create/deploy.parameters.json?token=***REMOVED***" \
...
then
	...
	az deployment sub create \
		...
		--template-uri "$githubUri/roleAssignment/customRoleDeploy.json?token=***REMOVED***" \
		--parameters \
...
then
	...
	az deployment sub create \
		--location eastus \
		--subscription="$subscriptionId" \
		--template-uri "$githubUri/roleAssignment/readerRoleDeploy.json?token=***REMOVED***" \
		--parameters \
			principalId="$principalId" \
